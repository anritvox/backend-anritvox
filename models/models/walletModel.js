...
async function initWalletTables() {
  ...
  const transactionQuery = `
    CREATE TABLE IF NOT EXISTS wallet_transactions (
      ...
      reference_id VARCHAR(100) NULL, -- Can link to an order_id or return_id
...
const WalletModel = {
  getWalletBalance: async (userId) => {
    // Create a wallet on the fly if it doesn't exist
  ...
  // Securely add or remove funds with an audit trail
  processTransaction: async (userId, amount, type, description, reference_id = null) => {
    const connection = await pool.getConnection();
    try {
      await connection.beginTransaction();

      // Lock the wallet row for this user to prevent race conditions (Double Spend prevention)
