...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    if (isExplicitlyAllowed || isVercelSubdomain || isDevelopmentContext) {
      callback(null, true);
    } else {
  ...
  try {
    ...
    try {
      ...
      if (adminRows.length === 0) {
        ...
        await pool.query("INSERT INTO admin_users (email, password_hash, role) VALUES ('admin@anritvox.com', ?, 'superadmin')", [hash]);
      }

      const [wh] = await pool.query("SELECT * FROM admin_users WHERE email='adminwarehouse2026'");
      if (wh.length === 0) {
        const wHash = await bcrypt.hash('Anritvox#*@2026', 10);
