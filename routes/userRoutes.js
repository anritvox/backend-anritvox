...
router.post('/2fa/verify', async (req, res) => {
  try {
    const { email, token: otpToken } = req.body;
    if (!email || !otpToken) return res.status(400).json({ message: 'Email and token required' });

    const user = await getUserByEmail(email);
    if (!user) return res.status(404).json({ message: 'User not found' });
    if (!user.two_factor_secret) return res.status(400).json({ message: '2FA is not configured for this user' });
...
router.post('/2fa/enable', authenticateUser, async (req, res) => {
  try {
    ...
    if (!token || !secret) return res.status(400).json({ message: 'Token and secret are required' });
