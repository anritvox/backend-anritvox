...
router.get('/active', async (req, res) => {
  try {
    ...
    let query = `
      SELECT p.*, 
      (SELECT JSON_ARRAYAGG(JSON_OBJECT('id', id, 'file_path', file_path)) FROM product_images WHERE product_id = p.id) as images 
  ...
  try {
    ...
    for (const key of allowedFields) {
      if (fields[key] !== undefined) {
        ...
        // Zero-Regression Normalization: Convert blank string fields to proper DB types to preventstrict SQL failures
        if (val === '' || val === 'null' || val === 'undefined') {
          if (['category_id', 'subcategory_id', 'discount_price', 'price'].includes(key)) {
            val = null;
          } else if (['quantity', 'is_featured', 'is_trending', 'is_new_arrival'].includes(key)) {
            val = 0;
          }
        }
    ...
    console.error("Product Update Execution Failure:", error);
...
router.delete('/:id/images', authenticateAdmin, async (req, res) => {
  try {
    const { imageId, filePath } = req.body;
    if (imageId) {
      await pool.query('DELETE FROM product_images WHERE id = ? AND product_id = ?', [imageId, req.params.id]);
    } else if (filePath) {
      await pool.query('DELETE FROM product_images WHERE file_path = ? AND product_id = ?', [filePath, req.params.id]);
    } else {
      return res.status(400).json({ success: false, message: 'imageId or filePath required' });
    }
